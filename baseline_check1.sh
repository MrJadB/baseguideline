...
can you fix script to work normally from problem below 
1, /etc/cron.allow no such a file or directory
2. /etc/securetty  no such a file or directory
3. /etc/sysconfig no such a file or directory
4. /etc/security/limits.d/* no such a file or directory
5. /etc/sysconfig/init no such a file or directory
6. Failed to get unit file state for telnet.socket, vsftpd.service,rsh.socket, rlogin.socket, rexec.socket, tftp.socket,dovecot.service, squid.service,kudzu.service
ChatGPT
To handle the issues where certain files or services might not exist, we can add checks to ensure the file exists before trying to read it, and handle the services gracefully if they are not found. Here is the updated script with these fixes:
...
if [ -f /etc/securetty ]; then
    ...
    echo "/etc/securetty does not exist" >> $OUTPUT_FILE
fi
add_separator

# 14. Set LILO/GRUB Password
echo "14. Set LILO/GRUB Password" >> $OUTPUT_FILE
grep password /boot/grub2/grub.cfg >> $OUTPUT_FILE
add_separator
...
if [ -f /etc/sysconfig/init ]; then
    ...
    echo "/etc/sysconfig/init does not exist" >> $OUTPUT_FILE
fi
add_separator
...
find / -perm -1000 -type d -print >> $OUTPUT_FILE
add_separator

# 19. Find All Unowned Files
echo "19. Find All Unowned Files" >> $OUTPUT_FILE
find / -nouser -o -nogroup >> $OUTPUT_FILE
add_separator
...
if [ -f /etc/security/limits.d/* ]; then
    grep -i core /etc/security/limits.d/* >> $OUTPUT_FILE
else
    echo "/etc/security/limits.d/* does not exist" >> $OUTPUT_FILE
fi
add_separator
...
if systemctl status telnet.socket &>/dev/null; then
    ...
    echo "telnet.socket does not exist" >> $OUTPUT_FILE
fi
add_separator

# 25. Only Enable ftp If Absolutely Necessary
echo "25. Only Enable ftp If Absolutely Necessary" >> $OUTPUT_FILE
if systemctl status vsftpd.service &>/dev/null; then
    systemctl is-enabled vsftpd.service >> $OUTPUT_FILE
else
    echo "vsftpd.service does not exist" >> $OUTPUT_FILE
fi
add_separator
...
for service in rsh.socket rlogin.socket rexec.socket; do
    if systemctl status $service &>/dev/null; then
        systemctl is-enabled $service >> $OUTPUT_FILE
    else
        echo "$service does not exist" >> $OUTPUT_FILE
    fi
done
add_separator
...
if systemctl status tftp.socket &>/dev/null; then
    systemctl is-enabled tftp.socket >> $OUTPUT_FILE
else
    echo "tftp.socket does not exist" >> $OUTPUT_FILE
fi
add_separator

# 28. Only Enable IMAP if Absolutely Necessary
echo "28. Only Enable IMAP if Absolutely Necessary" >> $OUTPUT_FILE
if systemctl status dovecot.service &>/dev/null; then
    systemctl is-enabled dovecot.service >> $OUTPUT_FILE
else
    echo "dovecot.service does not exist" >> $OUTPUT_FILE
fi
add_separator
...
if systemctl status squid.service &>/dev/null; then
    systemctl is-enabled squid.service >> $OUTPUT_FILE
else
    echo "squid.service does not exist" >> $OUTPUT_FILE
fi
add_separator
...
if systemctl status kudzu.service &>/dev/null; then
    systemctl is-enabled kudzu.service >> $OUTPUT_FILE
else
    echo "kudzu.service does not exist" >> $OUTPUT_FILE
fi
add_separator
...
if [ -f /etc/ftpusers ]; then
    ...
    echo "/etc/ftpusers does not exist" >> $OUTPUT_FILE
fi
add_separator
...
cat /etc/sysconfig/nfs | grep MOUNTD_PORT >> $OUTPUT_FILE
add_separator
...
grep -i nolisten /etc/X11/xinit/xserverrc >> $OUTPUT_FILE
add_separator
